...
accessinfo <- read.csv(file = "https://raw.githubusercontent.com/laurencehorton/inf1005-3/master/ati-2007-2014datafiles/file0607.csv")
accessinfo$x <- NULL
...
rm(accessinfo0607, raw_file0607, submedia, subacademics, subbusiness, suborgs, subpub, subdecline)

source0607 <- total[nrow(total),]
source0607$Total <- rowSums(source0607[, c(1:6)])
source0607$Year <- "2006-2007"
col_order <- c("Year", "Business", "Public", "Media", "Orginizations", "Academics", "Decline", "Total")
sum0607 <- source0607[, col_order]
names(sum0607)
